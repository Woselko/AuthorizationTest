...
public static class IConfigurationBuilderExtensions
{
    ...
    public static void AddClientConfigurations(this IConfigurationBuilder builder)
    {
        ...
        var sharedAssembly = Assembly.Load("Test.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("Test.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"Test.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = Assembly.Load("Test.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("Test.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"Test.Client.Core.appsettings.{AppEnvironment.Current}.json");
