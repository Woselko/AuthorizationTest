...
using Test.Shared;
using Test.Client.Core.Services;
...
public static partial class Program
{
    ...
    private static void ConfiureMiddlewares(this WebApplication app)
    {
        ...
        {
            options.InjectJavascript($"/_content/Test.Server.Api/scripts/swagger-utils.js?v={Environment.TickCount64}");
        });
        ...
        var blazorApp = app.MapRazorComponents<Components.App>()
            ...
            .AddAdditionalAssemblies(AssemblyLoadContext.Default.Assemblies.Where(asm => asm.GetName().Name?.Contains("Test") is true).Except([Assembly.GetExecutingAssembly()]).ToArray());
